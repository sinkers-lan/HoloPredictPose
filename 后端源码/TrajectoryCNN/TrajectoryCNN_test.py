...
import os.path
import numpy as np
from TrajectoryCNN.nets import TrajectoryCNN_model
from TrajectoryCNN.utils import metrics
from TrajectoryCNN.utils import recoverh36m_3d
        ...
        for i in range(n_gpu):
            with tf.device('/gpu:%d' % i):
                with tf.variable_scope(tf.get_variable_scope(),
                    ...
                    # define a model
                    output_list = TrajectoryCNN_model.TrajectoryCNN(
...
#测试主函数
def TraCNN_predict(model,pose_3D):
    print('test...')
    ...
    # print(n)
    # print(len(data))
    all_input = np.delete(all_input, range(n, len(data)), axis=0)
    ...
    save_data_1 = np.zeros(((n - n % batch_size) * steps, joints_number, 3))
    ...
    save_data_2[:, [0, 1, 4]] = data[:save_data_2.shape[0], [0, 1, 4]]
